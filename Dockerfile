...
RUN apt-get update && apt-get install build-essential ca-certificates cmake git libwxgtk3.0-gtk3-dev libcanberra-gtk3-module alsa-base pulseaudio libopencv-dev -y --no-install-recommends

RUN mkdir -p /src/PathPlanningVisualizer/build
COPY . /src/PathPlanningVisualizer/
RUN cd /src/PathPlanningVisualizer/build && cmake .. && cmake --build .
WORKDIR /src/PathPlanningVisualizer/build
